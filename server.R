...
shinyServer <- function(input, output, session) {
  ...
  output$stats1 <- renderText({
    ...
    snowR <- ifelse(input$snow != "" && input$trend, 
                    paste("<br><b style = 'color:blue;'>Snow conditions</b>
                          ...
                          "<br>R<sup>2</sup>:", signif(summary(fit_snow)$r.squared, digits = 2), "<br>"), "")
  ...
  output$plot2 <- renderPlotly({
    ...
    p2 <- plot_ly() %>%
      add_markers(x = ~phen[, snowVar], 
                  y = ~phen[, spVar], 
                  name = "species", 
                  showlegend = F, 
                  text = paste0("Year: ", phen$Year),
  ...
  output$plot3 <- renderPlotly({
    ...
    p3 <- plot_ly() %>%
      add_markers(x = ~phen[, weatherVar], 
                  y = ~phen[, spVar], 
                  name = "species", 
                  showlegend = F,
                  text = paste0("Year: ", phen$Year),
                  hovertemplate = "%{text} <br>(%{x:.2f}, %{y})") %>%
